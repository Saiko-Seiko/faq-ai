...
## 削除請求の確認（Stage 4 の後追い）

`db/003_privacy.sql` を適用済みであれば、追加のマイグレーションは要らない。
届いた削除請求を人事が見る画面が無かったため、`/dashboard` の上部に追加した。

確認：

- [ ] `/privacy` の削除請求フォームから送信する
- [ ] `/dashboard` の上部に「未対応の削除請求 1件」が出る
- [ ] 「内容を見る」で氏名・連絡先・依頼内容が読める
- [ ] 「対応しました」→ 一覧から消え、`deletion_requests` の氏名・連絡先が NULL になる
- [ ] 請求が0件のときは、パネルそのものが出ない
- [ ] 閲覧のみの担当者では、ボタンが押せない
...
デプロイと初期設定が済んだら、[07-operations.md](07-operations.md)
（人事ご担当者向けの運用マニュアル）をお渡しする。
画面ごとの操作、権限の考え方、削除請求への対応手順、
月次で見ていただきたい項目までを1本にまとめてある。

引き渡し前に、こちらで済ませておくこと：

- [ ] `/content` の「サンプルの会社情報が表示されています」の警告が**消えている**
      （消えていなければ、架空の会社の情報が応募者に見えている）
- [ ] 最初の管理者を登録し、**`HR_ACCESS_TOKEN` を環境変数から削除した**
- [ ] `PRIVACY_CONTACT` に実際の問い合わせ先が入っている
- [ ] `RETENTION_DAYS_*` がクライアントの社内規程と一致している
- [ ] `/api/cleanup` の Cron が動いている（Vercel のログで確認）
